...
#' @export
#' @examples
#' data_microarray <- prep_norm_microarray(data_microarray, varFilter = TRUE,
#'                                         featureFilter = TRUE,
...
#' @export
#' @examples
#' data_RNASeq <- prep_norm_rna(data_RNASeq, usFilter = TRUE, method = "TMM")
#' data_RNASeq
...
prep_metSB <- function(object, filterMV = TRUE, filterF = TRUE, filterRSD = TRUE,
  ...
  if(filterRSD == TRUE) {
    ...
    if(is.null(QClab) == TRUE) stop("must select a quality control class label")
  ...
  if(filterB == TRUE) {
    ...
    if(is.nul(Blab) == TRUE) stop("must select a blank class label")
...
#' @param impute,reoutlier Lógico, si es \code{TRUE} se realizará la imputación
#'   de valores perdidos (para \code{impute} o la detección y eliminación de
#'   outliers (para \code{reoutliers}).
...
met_imp_norm <- function(object, impute = TRUE, coff = 20, immethod = "none",
  ...
  if(routliers == TRUE) {
    ...
    if(is.null(oumethod) == TRUE) stop("must select a remove outliers method")
