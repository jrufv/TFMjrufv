...
plotTFM <- function (object, plot) {
  ...
  if(plot == "PCA") {
    ...
    pcmat <- prcomp(t(dat_mat), scale=FALSE)
    ...
    if(class(object) == "DGEList") {
      ...
      ggplot2::ggplot(datalibsize,
                      ggplot2::aes(x = samples, y = lib.size, fill = group)) +
        ggplot2::theme_bw() +
        ggplot2::geom_bar(stat = "identity", width = 0.5) +
        ggplot2::geom_text(ggplot2::aes(label = lib.size), vjust = 1.5, color = "white",
                           size = 3.5) +
    ...
    if(class(object) == "OnDiskMSnExp" | class(object) ==  "MSnExp") {
      ...
      legend("topright", fill = grDevices::topo.colors(ngroup(object)), legend = lev)
    ...
    legend("bottomleft", fill = grDevices::topo.colors(ngroup(object)), legend = lev)
